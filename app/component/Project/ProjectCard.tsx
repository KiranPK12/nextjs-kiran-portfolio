...
const ProjectCard = ({ title, description, tags, imageUrl }: ProjectCard) => {
  ...
  return (
    ...
    >
      <section className=" bg-gray-100 max-w-[42rem] border border-black/5 overflow-hidden sm:pr-8 relative h-[18rem] sm:h-[20rem] hover:bg-gray-200 transition rounded-lg dark:bg-white/10 dark:hover:bg-white/20">
        <div className="pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full sm:group-even:ml-[20rem] gap-7 sm:gap-0z">
          ...
          <ul className="flex gap-2 mt-2 flex-wrap sm:mt-auto">
            {tags.map((tag, index) => (
              <li
                key={index}
                className="bg-black/[0.7] px-3 py-1 text-[0.7rem] uppercase tracking-wider rounded-full text-white dark:text-white/70"
              >
                {tag}
              </li>
            ))}
